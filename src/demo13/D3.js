      ...
      <div className="demo3">
        {
          order.map((_, idx) => {
            ...
            let x;
            let y;
            const visualPosition = order.indexOf(idx);
            if (idx === lastPress && isPressed) {
              [x, y] = mouseXY;
              style = {
                translateX: x,
                translateY: y,
              ...
              <Motion key={idx} style={style}>
                {
                  ({translateX, translateY, scale, boxShadow}) =>
                    <div
                      onMouseDown={e => this.handleMouseDown(idx, [x, y], e)}
                      onTouchStart={e => this.handleTouchStart(idx, [x, y], e)}
                      className="demo3-ball"
                      style={{
                        backgroundColor: allColors[idx],
                        transform: `translate3d(${translateX}px, ${translateY}px, 0) scale(${scale})`,
                        zIndex: idx === lastPress ? 99 : visualPosition,
